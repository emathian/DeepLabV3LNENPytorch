...
############################################################################################################### Emilie encoder - decoder deeplab v3+
##############################################################################################################

def define_Enc(netEnc, init_type, init_gain, gpu_ids=[]):
    """Create a generator
    ...
    Parameters:
        ...
        output_nc (int) -- the number of channels in output images
        ngf (int) -- the number of filters in the last conv layer
        netEnc(str) -- the architecture's name: resnet_9blocks | resnet_6blocks | unet_256 | unet_128
        use_dropout (bool) -- if use dropout layers.
        init_type (str)    -- the name of our initialization method.
        gpu_ids (int list) -- which GPUs the network runs on: e.g., 0,1,2
   ...
   """
    ...
    if netEnc == 'resnet50dilated':
         orig_resnet = resnet.__dict__['resnet50'](pretrained=False)
         net_encoder = ResNet50Dilated(orig_resnet)
    return init_net(net_encoder, init_type, init_gain, gpu_ids)



def define_Dec(netDec, init_type, init_gain, fc_dim, num_class,
                   use_softmax=False ,gpu_ids=[]):
    """Create a generator
   ...
   """
    ...
    if netDec == 'ppm_deepsup':
         net_decoder = PPMDeepsup(num_class, fc_dim, use_softmax)
    return init_net(net_decoder, init_type, init_gain, gpu_ids)
